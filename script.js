function lerArquivoCSV(callback) {
  ...
  function lerArquivoCSV2(callback) {
    var xhr = new XMLHttpRequest();
    xhr.onreadystatechange = function () {
      if (xhr.readyState === XMLHttpRequest.DONE) {
        if (xhr.status === 200) {
    ...
    xhr.open('GET', 'temporada_data.csv', true); // Caminho relativo ao HTML
    xhr.send();
  }
  
  function criarGraficosAnimados2(data, maxItensPorGrafico) {
    var linhas = data.split('\n');
    var cabecalho = linhas[0].split(',');
    var dataset = [];
    ...
    for (var k = 0; k < numGraficos; k++) {
      ...
      var canvas = document.createElement('canvas');
      canvas.id = 'animatedChart' + k;
      document.getElementById('chartsContainer2017').appendChild(canvas);
      ...
      var chart = new Chart(ctx, {
        ...
        data: {
          ...
          datasets: [{
            label: 'Quantidade',
            data: dadosGrafico.map(item => parseInt(item.quantidade)), // Converte para número inteiro
            backgroundColor: 'rgba(75, 192, 192, 0.2)',
            borderColor: 'rgba(75, 192, 192, 1)',
            borderWidth: 1
          }]
  ...
  lerArquivoCSV2(function(data) {
    criarGraficosAnimados2(data, 25); // Especifique o número desejado de itens por gráfico
  });